...
## 9. Workbench Hardware-in-the-Loop (HIL) E2E Testing (`tools/canbox_bench_e2e.py`)

If you have a physical automotive test bench with real hardware (BSI, BMS, instrument cluster, steering column stalk, buttons, doors, etc.) connected to Linux via SLCAN / SocketCAN (`can0`) and wired to the Android Head Unit over USB-to-UART (`/dev/ttyUSB1` @ 38400 baud 8N1), use the specialized E2E testing tool:

```bash
# 1. Quick Start: Run bench E2E test with default parameters (can0, /dev/ttyUSB1 @ 38400, Hiworld protocol)
python3 tools/canbox_bench_e2e.py

# 2. Automatically setup SLCAN USB adapter on /dev/ttyUSB0 (500kbps CAN @ 1Mbaud UART)
python3 tools/canbox_bench_e2e.py --setup-slcan --slcan-dev /dev/ttyUSB0

# 3. Specify custom Head Unit port, protocol, and vehicle profile
python3 tools/canbox_bench_e2e.py \
  --can-iface can0 \
  --hu-port /dev/ttyUSB1 \
  --hu-baud 38400 \
...
# 4. Record live bench CAN frames to a CSV file while testing
python3 tools/canbox_bench_e2e.py --record-can bench_capture_$(date +%Y%m%d_%H%M%S).csv
```

### 9.1 Interactive Live Dashboard

When running `canbox_bench_e2e.py`, an interactive terminal dashboard displays decoded vehicle states in real time:

- **Telemetry & Cluster:** Speed (km/h), RPM, Steering Wheel Angle ($\pm^\circ$), Stalk & Wheel button presses.
- **Body & Doors:** Individual door status (Driver, Passenger, Rear-L, Rear-R, Trunk, Hood), Reverse Gear, Handbrake.
- **Lights & HVAC:** Side lights, dipped beam, high beam, fogs; Dual-zone climate temperatures, fan speed, A/C state.
- **Parking Radar:** Ultrasonic obstacle levels (Front & Rear).
- **CAN Bus Statistics:** Frame rate (FPS), total frame count, and recent event log.
